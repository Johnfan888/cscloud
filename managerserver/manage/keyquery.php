...
else
{
   ...
   setcookie("admin[username]", $username);
	...
	$sql="select   *  ,count(*)  from   tb_file_all   where   name like '%".$keywords."%' and user_id=(select user_id from members where username='".$_COOKIE['admin']['username']."')  group by parent_id, name";
	...
	{
	  ShowTreeMenu($user->Con1,$result,$layer,$user_id,$_GET["version_flag"],$keywords,$getname,$getpid,$menu_flag,$ID);
	}             
	else
	{
	echo "没有您要找的文件！" ;
  ...
  //layer：需要显示的菜单的级数             
  //=============================================             
	function   ShowTreeMenu($Con,$result,$layer,$user_id,$version_flag,$keywords,$getname,$getpid,$menu_flag)           
{             
		//取得需要显示的菜单的项目数             
				 ...
				 //----------------------------------------------------------------
					...
					{//说明是第一次访问该页
						 echo   "<tr><td align='center'><img	src='file.gif'   border='0'></td>
						  <td>".$menu['name']."<a href='keyquery.php?version_flag=1&name=".$menu['name']."&pid=".$menu['parent_id']."&keywords=".$keywords."'>详细信息</a></td>";  
					}
					else
					{//说明是通过点击详细信息而访问的该页
						 ...
						 {//关闭
						  echo   "<tr><td align='center'><img	src='file.gif'   border='0'></td>
						  <td>".$menu['name']."<a href='keyquery.php?version_flag=0&name=".$menu['name']."&pid=".$menu['parent_id']."&keywords=".$keywords."'>详细信息</a></td>";  
						 }else{//其他的
						  echo   "<tr><td align='center'><img	src='file.gif'   border='0'></td>
						  <td>".$menu['name']."<a href='keyquery.php?version_flag=1&name=".$menu['name']."&pid=".$menu['parent_id']."&keywords=".$keywords."'>详细信息</a></td>";  
						}
