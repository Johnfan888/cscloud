        ...
        if "swp" not in event.name:
            if "swx" not in event.name:
                if "~" not in event.name:
                    ...
                    item_event = "已创建"
                    #作为子线程运行
                    # threading.Thread(target=self.exec_api, args=(
                    #     event, method, filename, item_event,)).start()
                    name['p' + str(self.nn)] = multiprocessing.Process(target=self.exec_api, args=(
                        event, method, filename, item_event,))  # 单个参数时候逗号必须有代表传递的是数组
                    name['p' + str(self.nn)].start()
                    self.nn += 1
                    # if self.nn > 5:
                    #     name['p' + str(self.nn)].join()
                    # print self.nn
        ...
        if "swp" not in event.name:
            if "swx" not in event.name:
                if "~" not in event.name:
                    ...
                    filename = os.path.join(event.path, event.name)
                    item_event = "文件已移动来"
                    name1['m' + str(self.nn)] = multiprocessing.Process(target=self.exec_api, args=(
