...
class EventHandler(ProcessEvent):
    ...
    def exec_api(self, event):
        oid_status, oid_output = commands.getstatusoutput(
            "getfattr -n user.oid  '%s' --only-values  --absolute-names" % (self.filename))
        if oid_status == 0:
            oid = oid_output
            print oid
        else:
        ...
        status, output = commands.getstatusoutput(
            "php /csc/csc_client_api.php '%s' '%s' '%s' '%s'" % (
                self.method, urllib.quote(self.filename), oid, self.ms_ip))  # urllib.quote(self.filename)
        print output
        if status == 0:
            print "上传结束"
