...
class EventHandler_create(ProcessEvent):
    ...
    def __init__(self):
        ...
        self.ms_ip = "192.168.1.224"
        self.pool = multiprocessing.Pool(processes=2)#设置进程池的大小
        ...
        if "swp" not in event.name:
            if "swx" not in event.name:
                if "~" not in event.name:
                    ...
                    item_event = "已创建"
                    self.pool.apply_async(func=exec_api, args=(method, filename, item_event,self.obs,self.ms_ip))#加入池
