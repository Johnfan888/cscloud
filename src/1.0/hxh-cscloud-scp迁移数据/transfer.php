<?php
//echo $_GET["originip"]."<br>";
//echo $_GET["targetip"]."<br>";
$stage=$_GET['stage'];
...
require("../include/configure_class.php"); //文件路径不正确  不存在此文件
...
{      
		...
		$fp=fopen("/var/log/csc/stage.txt","w"); //不存在
        fwrite($fp,$stage."\n");
		fclose($fp);
...
else if($stage==3)//说明源服务器接收密码成功，开始发送文件信息
{
		...
		if($fileowner=='0')
		{
			...
			//不需要迁移，返回configer，将数据库的相应字段回滚
				...
				$fp=fopen("/var/log/csc/stage.txt","a"); //文件不存在
				fwrite($fp,$get_url."\n");
				fclose($fp);
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true) ; 
				curl_setopt($ch, CURLOPT_BINARYTRANSFER, true) ; 
...
//在这里决定要迁移哪些文件，然后将文件的location传给filelocation，如果是多文件，那么传数组可以通过地址栏传送参数也可以通过post传递，地址栏传送时有长度限制1024k，所以做的时候我觉得还是用post方法好一些，
			$clientstage=$_GET['clientstage']+1;//$clientstage=2
			 ...
			 "clientstage"=>$clientstage //$clientstage=2
			   	...
			   	$post_data2=select($fileowner,$originip); //返回路径
...
//将该文件的信息插入到迁移的日志文件中			  
			$sql=$showtime.",".$array["id"][$i].",".$originip.",".$targetip.",".$menu["locationpath"].",".$array["newpath"][$i]."\n"; 
			$ff=fopen("/var/log/csc/transfer.txt","w");
			fwrite($ff,$sql);
			fclose($sql);
			  
//修改tb_file_location的serverip信息
			  $sql="update tb_file_location set serverip='".$targetip."',locationpath='".$array["newpath"][$i]."' where id='".$array["id"][$i]."'";
			   mysql_query($sql,$user->Con1);
			   
//修改该文件的所有父目录的serverip信息
			$sql="select parent_id from tb_file_all where id='".$array["id"][$i]."'";
			$res=mysql_query($sql,$user->Con1);
			$arr=mysql_fetch_array($res);
			$id=$arr["parent_id"];
			while($id!=0)
			{
				$sql="update tb_file_location set serverip='".$targetip."' where id='".$id."'";
				mysql_query($sql,$user->Con1);
				
				$sql="select parent_id from tb_file_all where id='".$id."'";
				$res=mysql_query($sql,$user->Con1);
				$arr=mysql_fetch_array($res);
				$id=$arr["parent_id"];
			}
...
//更新filesize的信息	
			   $sql="update filesize set serverip='".$targetip."' where username='".selectuser($originip)."'";
			   mysql_query($sql,$user->Con1);
			   
			   //做迁移文件后的操作
			 $fp1=fopen("/var/log/csc/fileid.txt","r");
...
//将传输完成的消息告诉configmanager
			$get_url="http://".$configmanagerip."/configmanager/write_flag.php?flag=2";
			$ch = curl_init($get_url) ; 
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true) ; 
			curl_setopt($ch, CURLOPT_BINARYTRANSFER, true) ; 
			$output = curl_exec($ch) ; 
...
function select($fileowner,$originip) //传递参数用户id，源IP
{
	...
	$sql="select distinct file_path from T_FileLocation where server_ip='".$originip."' and  user_id='".$fileowner."'";//选择需要迁移的文件，目录不需迁移
...
{
	require_once("../include/comment.php"); //文件不存在
	require_once("../include/user.class.php"); //文件不存在
