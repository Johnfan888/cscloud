	...
	if($db->NumRowsWithoutSql() > 0)
	{
		...
		if($fileinfo)
		{
			...
			else
			{
			 ...
			 $url = "http://{$ip}/www/ds_online_play.php" ; //下载的地址
			 $name = rawurlencode($file['file_name']);//转化为适合url传输的格式 
			...
			if($file['version'] > 1)
			{
				...
				$bid = $base['file_id'];
				header("Location:{$url}?path={$path}&user={$_COOKIE['id']}&id={$file['file_id']}&name={$name}&base={$bid}"); //用header重定向
			}
			else
			{
